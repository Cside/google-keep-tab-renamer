...
const main = () => {
  if (['#home', '#search'].some((hash) => location.hash === hash)) {
    ...
    if (location.hash.startsWith('#label/')) {
      document.title = decodeURIComponent(location.hash).replace(/^#label\//, '');
    } else if (location.hash.startsWith('#NOTE/')) {
      const title =
        document.querySelector<HTMLElement>('[contenteditable=true][spellcheck=true]')
      ...
      if ($heading.textContent === null || $heading.textContent?.length === 0) return;
